...
/**
 * Lista de Fornecedores (T-M5-9, CA-11/CA-13/CA-14). Espelho fiel de `clientes.spec.ts`. Dados
 * FICTÍCIOS (LGPD, §3.3): "Flores do Vale", `@exemplo.com.br`, `(11) 90000-0000` — nunca PII real.
 */
describe('Fornecedores (lista — T-M5-9, CA-11/CA-13/CA-14)', () => {
  ...
  function pagina(conteudo: Fornecedor[], total = conteudo.length): PaginaResponse<Fornecedor> {
    return {
      conteudo,
      pagina: 0,
      tamanho: 12,
  ...
  function iniciar(conteudo: Fornecedor[], total = conteudo.length): ComponentFixture<Fornecedores> {
  ...
  it('renderiza um único <h1> "Fornecedores" (a11y — um h1 por página)', () => {
    const el = iniciar([base]).nativeElement as HTMLElement;
    const titulos = el.querySelectorAll('h1');
    expect(titulos.length).toBe(1);
    expect(titulos[0].textContent).toContain('Fornecedores');
  });

  it('um card por fornecedor com monograma, nome, telefone e e-mail', () => {
    const el = iniciar([base]).nativeElement as HTMLElement;
    expect(el.querySelectorAll('.ficha').length).toBe(1);
    expect(el.querySelector('.ficha__monograma')?.textContent?.trim()).toBe('F'); // inicial
    expect(el.textContent).toContain('Flores do Vale');
    expect(el.textContent).toContain('(11) 90000-0000');
    expect(el.textContent).toContain('contato@exemplo.com.br');
    expect(el.querySelector('.ficha__obs')?.textContent).toContain('terças e sextas');
  });

  it('sem telefone e sem e-mail mostra a linha honesta de "sem contato"', () => {
    const semContato: Fornecedor = { ...base, telefone: null, email: null };
  ...
  it('filtro por nome com debounce (~300ms) refaz a busca com o param nome (CA-11)', fakeAsync(() => {
    ...
    const req = httpMock.expectOne((r) => r.url === BASE && r.params.get('nome') === 'fl');
  ...
  it('USER não vê "Novo fornecedor"/Editar/Excluir (só lê — CA-13)', () => {
    ehAdmin.set(false);
    const el = iniciar([base]).nativeElement as HTMLElement;
  ...
  it('ADMIN vê "Novo fornecedor" na placa e Editar/Excluir no card (CA-13)', () => {
    ehAdmin.set(true);
    const el = iniciar([base]).nativeElement as HTMLElement;
    expect(el.querySelector('.placa__acao')?.textContent).toContain('Novo fornecedor');
    expect(el.querySelector('.ficha__editar')).toBeTruthy();
    expect(el.querySelector('.ficha__excluir')).toBeTruthy();
    expect(el.querySelectorAll('h1').length).toBe(1);
  });

  it('ADMIN clicando em "Novo fornecedor" arma o gancho do form (T-M5-10) sem quebrar', () => {
    ehAdmin.set(true);
    const fixture = iniciar([base]);
    ...
    expect(comp.fornecedorEmEdicao()).toBeNull();
    // o overlay <app-fornecedor-form> só chega na T-M5-10: ainda não renderiza nada.
    expect(fixture.nativeElement.querySelector('app-fornecedor-form')).toBeNull();
  });
